...
abstract class VfsUtilsFake {
    ...
    static {
        ClassLoader loader = VfsUtilsFake.class.getClassLoader();
        try {
            Class<?> vfsClass = loader.loadClass(VFS3_PKG + VFS_NAME);
            VFS_METHOD_GET_ROOT_URL = ReflectionUtilsFake.findMethod(vfsClass, "getChild", URL.class);
            VFS_METHOD_GET_ROOT_URI = ReflectionUtilsFake.findMethod(vfsClass, "getChild", URI.class);

            Class<?> virtualFile = loader.loadClass(VFS3_PKG + "VirtualFile");
            VIRTUAL_FILE_METHOD_EXISTS = ReflectionUtilsFake.findMethod(virtualFile, "exists");
            VIRTUAL_FILE_METHOD_GET_INPUT_STREAM = ReflectionUtilsFake.findMethod(virtualFile, "openStream");
            VIRTUAL_FILE_METHOD_GET_SIZE = ReflectionUtilsFake.findMethod(virtualFile, "getSize");
            VIRTUAL_FILE_METHOD_GET_LAST_MODIFIED = ReflectionUtilsFake.findMethod(virtualFile, "getLastModified");
            VIRTUAL_FILE_METHOD_TO_URI = ReflectionUtilsFake.findMethod(virtualFile, "toURI");
            VIRTUAL_FILE_METHOD_TO_URL = ReflectionUtilsFake.findMethod(virtualFile, "toURL");
            VIRTUAL_FILE_METHOD_GET_NAME = ReflectionUtilsFake.findMethod(virtualFile, "getName");
            VIRTUAL_FILE_METHOD_GET_PATH_NAME = ReflectionUtilsFake.findMethod(virtualFile, "getPathName");
            VIRTUAL_FILE_METHOD_GET_PHYSICAL_FILE = ReflectionUtilsFake.findMethod(virtualFile, "getPhysicalFile");
            VIRTUAL_FILE_METHOD_GET_CHILD = ReflectionUtilsFake.findMethod(virtualFile, "getChild", String.class);

            VIRTUAL_FILE_VISITOR_INTERFACE = loader.loadClass(VFS3_PKG + "VirtualFileVisitor");
            VIRTUAL_FILE_METHOD_VISIT = ReflectionUtilsFake.findMethod(virtualFile, "visit", VIRTUAL_FILE_VISITOR_INTERFACE);

            Class<?> visitorAttributesClass = loader.loadClass(VFS3_PKG + "VisitorAttributes");
            VISITOR_ATTRIBUTES_FIELD_RECURSE = ReflectionUtilsFake.findField(visitorAttributesClass, "RECURSE");
        } catch (Throwable ex) {
    ...
    protected static Object invokeVfsMethod(Method method, Object target, Object... args) throws IOException {
        try {
            ...
            ReflectionUtilsFake.handleInvocationTargetException(ex);
        } catch (Exception ex) {
            ReflectionUtilsFake.handleReflectionException(ex);
        }
    ...
    protected static Object doGetVisitorAttribute() {
        return ReflectionUtilsFake.getField(VISITOR_ATTRIBUTES_FIELD_RECURSE, null);
    }

    protected static String doGetPath(Object resource) {
        return (String) ReflectionUtilsFake.invokeMethod(VIRTUAL_FILE_METHOD_GET_PATH_NAME, resource);
    }
}
